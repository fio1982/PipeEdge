...
from models import MLP, CNNMnist, CNNCifar, Cifar10CNN, AlexNetMnist
...
from mp.alex.l2.alex import alexnetl2_1
from mp.alex.l4.alex import alexnetl4_1 
from mp.resnet.l2.resnet import resnet50l2_1
from mp.resnet.l4.resnet import resnet50l4_1
from mp.vgg.l2.vgg import vgg16l2_1
from mp.vgg.l4.vgg16 import vgg16l4_1

from global_sets import MODEL_PARTITION_NAME
...
def krum_distance(localModels, peers, model=None):
    ...
    for i in range(peers):
        ...
        b = None
        if model is None:
            a = choose_model_partition(MODEL_PARTITION_NAME)
        else:
            a = model
        ...
        for j in range(i+1, peers):
            if localModels[j+1] == None:
                continue
            if model is None:
                b = choose_model_partition(MODEL_PARTITION_NAME)
            else:
                b = model
...
def multi_krum(localModels, user_count, f, peers, model=None):
    distances = krum_distance(localModels, peers, model)
    sel_set = []
...
def choose_model_partition(model='alexl2_1'):
    if model ==  'alexl2_1':
        return alexnetl2_1()
    elif model == 'alexl4_1':
        return alexnetl4_1() 
    elif model == 'resnet50l2_1':
        return resnet50l2_1()   
    elif model == 'resnet50l4_1':
        return resnet50l4_1()
    elif model == 'vgg16l2_1':
        return vgg16l4_1() 
    elif model == 'vgg16l4_1':
        return vgg16l4_1() 
    # elif model == 'res':
    #     return resnet50()
