...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export EDGE_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.example.com/ca && ls *_sk)
    # export EDGE_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.example.com/ca && ls *_sk)
  fi
  ...
  sleep 5
  echo "Sleeping 120s to allow $CONSENSUS_TYPE cluster to complete booting"
  sleep 120
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/pbft-network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/pbft-network/ledgers-backup
...
function generateChannelArtifacts() {
  ...
  echo "##########################################################"
  set -x
  configtxgen -profile Genesis -channelID $SYS_CHANNEL -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  # for peers in different orgs discover and communicate
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for Org1MSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile Channel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
