...
func ReadConfig() *SharedConfig {
	...
	// calc the fault num
	// if len(t) < 3 {
	// 	log.Fatalf("[Config Error] the incorrent node num : %d, need at least 3", len(t))
	// 	return nil
	// }
	if len(t) % 3 != 1 {
...
func GetConfigureID() (id int, err error){
	...
	rawID  := os.Getenv("SESTUFF_NODE_ID")
...
func GetConfigureTable() (map[int]string, error){
	...
	rawTable  := os.Getenv("SESTUFF_NODE_TABLE")
	...
	log.Println("table: ", tables)
	// number of node is not 3f + 1
	// if len(tables) < 3 {
...
func GetConfigurePort() (port int, err error){
	...
	rawPort := os.Getenv("SESTUFF_LISTEN_PORT")
