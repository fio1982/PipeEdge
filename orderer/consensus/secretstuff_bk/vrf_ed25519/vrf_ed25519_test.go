...
func TestGeScalarMult(t *testing.T) {
	...
	x := vrf_ed25519.ExpandSecret(sk)
	h1 := vrf_ed25519.GeScalarMult(c, x)
	h1.ToBytes(&res1)
	// copy c to h2
...
func TestGeAdd(t *testing.T) {
	...
	c1 := vrf_ed25519.ECVRF_hash_to_curve([]byte(message), tmp[:])

	io.ReadFull(rand.Reader, tmp[:])
	a1 := vrf_ed25519.ExpandSecret(tmp[:])
	io.ReadFull(rand.Reader, tmp[:])
	a2 := vrf_ed25519.ExpandSecret(tmp[:])
	...
	h3 := vrf_ed25519.GeAdd(vrf_ed25519.GeScalarMult(c1, a1), vrf_ed25519.GeScalarMult(c1, a2))
	h3.ToBytes(&res2)
	if !bytes.Equal(res1[:], res2[:]) {
...
func DoTestECVRF(t *testing.T, pk, sk []byte, msg []byte, verbose bool) {
	pi, err := vrf_ed25519.ECVRF_prove(pk, sk, msg[:])
	if err != nil {
		t.Fatal(err)
	}
	res, err := vrf_ed25519.ECVRF_verify(pk, pi, msg[:])
	if err != nil {
		t.Fatal(err)
	}
	if !res {
	...
	if verbose {
		...
		fmt.Printf("vrf: %s\n", hex.EncodeToString(vrf_ed25519.ECVRF_proof2hash(pi)))

		r, c, s, err := vrf_ed25519.ECVRF_decode_proof(pi)
		if err != nil {
			t.Fatal(err)
		}
		// u = (g^x)^c * g^s = P^c * g^s
		var u ed1.ProjectiveGroupElement
		P := vrf_ed25519.OS2ECP(pk, pk[31] >> 7)
		ed1.GeDoubleScalarMultVartime(&u, c, P, s)
		fmt.Printf("r: %s\n", hex.EncodeToString(vrf_ed25519.ECP2OS(r)))
		fmt.Printf("c: %s\n", hex.EncodeToString(c[:]))
		fmt.Printf("s: %s\n", hex.EncodeToString(s[:]))
		fmt.Printf("u: %s\n", hex.EncodeToString(vrf_ed25519.ECP2OSProj(&u)))
	}
}
...
func TestECVRFOnce(t *testing.T) {
	...
	fmt.Printf("h: %s\n", hex.EncodeToString(vrf_ed25519.ECP2OS(h)))
}
...
// 		P := vrf_ed25519.ECVRF_hash_to_curve(m[:], pk)
// 		// test P on curve by P^order = infinity
// 		var infs [32]byte
// 		inf := vrf_ed25519.GeScalarMult(P, vrf_ed25519.IP2F(q))
// 		inf.ToBytes(&infs)
// 		if infs != [32]byte{1} {
