...
func GetDelays(filename string, id message.Identify) []string{
	delayMap := ReadNetworkFile(filename)
	var delay []string
	for index, delays :=range delayMap {
		if uint64(index) == AsUint64(id) {
			delay = delays
			break;
		}
		// log.Println("delay: ", delays)
	}
...
func (n *Node) RemoveFile() {
	filename := strconv.FormatUint(AsUint64(n.id), 10) + "_consensus_time.log"
...
func (n *Node) WriteTime(filename string, writeType string) {
	filename = strconv.FormatUint(AsUint64(n.id), 10) + "_" + filename
	path, err := os.Getwd()
	if err != nil {
	    log.Println(err)
	}
...
func ReadNetworkFile(filename string) map[int][]string {
	path, err := os.Getwd()
	if err != nil {
	    log.Println(err)
	}
	...
	delayMap := make(map[int][]string)
	for index,delays := range arr {	
		re := regexp.MustCompile(`\d+`)
		delays := re.FindAllString(delays, -1)
		delayMap[index] = delays	
